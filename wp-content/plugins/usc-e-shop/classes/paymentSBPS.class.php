	...
	public function auto_settlement_mail( $member_id, $order_id, $response_data, $continue_data ) {
		...
		if ( 'on' == $acting_opts['auto_settlement_mail'] ) {
			...
			if ( usces_is_html_mail() ) {
				...
				$message .= '<tr><td>';
				$m_header = apply_filters( 'usces_filter_sbps_auto_settlement_mail_header', $mail_header, $member_id, $order_id, $order_data, $response_data, $continue_data );
				...
				$message .= apply_filters( 'usces_filter_sbps_auto_settlement_mail_body', $mail_body, $member_id, $order_id, $order_data, $response_data, $continue_data );
				...
				$m_footer    = apply_filters( 'usces_filter_sbps_auto_settlement_mail_footer', $mail_footer, $member_id, $order_id, $order_data, $response_data, $continue_data );
				$message    .= '<tr><td style="padding:20px 30px;">';
	...
	public function auto_settlement_error_mail( $member_id, $order_id, $response_data, $continue_data ) {
		...
		if ( 'on' == $acting_opts['auto_settlement_mail'] ) {
			...
			if ( usces_is_html_mail() ) {
				...
				$message .= '<table style="font-size:15px; margin-bottom: 10px;" border="0" width="540" cellspacing="0" cellpadding="0" align="center" bgcolor="#ffffff"><tbody>';
				$message .= '<tr><td>';
				$m_header = apply_filters( 'usces_filter_sbps_auto_settlement_error_mail_header', $mail_header, $member_id, $order_id, $order_data, $response_data, $continue_data );
				$message .= do_shortcode( wpautop( $m_header ) );
				$message .= '</td></tr>';
				$message .= '</tbody></table>';
				$message .= '</td></tr>';
				...
				$message .= __( 'Details of auto-recurring billing process', 'usces' );
				$message .= '</caption></table>';
				$message .= apply_filters( 'usces_filter_sbps_auto_settlement_error_mail_body', $mail_body, $member_id, $order_id, $order_data, $response_data, $continue_data );
				$message .= '<hr style="margin: 50px 0 0; border-style: none; border-top: 3px solid #777;" />';
				$message .= '</td></tr>';
				// add footer.
				$mail_footer = '<p>' . __( 'If you have any questions, please contact us.', 'usces' ) . '</p>' . $mail_data['footer']['thankyou'];
				$m_footer    = apply_filters( 'usces_filter_sbps_auto_settlement_error_mail_footer', $mail_footer, $member_id, $order_id, $order_data, $response_data, $continue_data );
				$message    .= '<tr><td style="padding:20px 30px;">';
				$message    .= do_shortcode( wpautop( $m_footer ) );
				$message    .= '</td></tr>';
				$message    .= '</tbody></table>';
