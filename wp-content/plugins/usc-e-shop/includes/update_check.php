...
function usces_check_for_updates(){
	...
	if( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ! empty( $_GET['puc_slug'] ) ){
		...
		$filter_handle = 'puc_manual_check_message-' . $_GET['puc_slug'];
	...
	if( empty( $check_info ) ){
		...
		switch( $check_info ){
			case '422 Unprocessable Entity':
				$message = __autoload( 'Update Right is invalid. The corresponding plug-in does not exist.', 'usces' ) . '（ERROR422）';
			break;
