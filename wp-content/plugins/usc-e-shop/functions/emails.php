...
function usces_order_confirm_message( $order_id ) {
	...
	if ( usces_is_html_mail() ) {
		...
		if ( '' === $mail_mode ) {
			...
			$message .= do_shortcode( wpautop( $mail_data['header'][ $mail_mode ] ) );
			$message .= '</td></tr>';
			// add body
			$message .= '<tr><td style="padding:20px 30px;">';
			$message .= apply_filters( 'usces_filter_order_confirm_mail_body_after', $msg_body, $data );
		...
		if ( '' === $mail_mode ) {
			...
			$message .= do_shortcode( $mail_data['header'][ $mail_mode ] ) . apply_filters( 'usces_filter_order_confirm_mail_body_after', $msg_body, $data ) . do_shortcode( $mail_data['footer'][ $mail_mode ] );
		}
	}
...
function usces_send_regmembermail( $user ) {
	...
	if ( usces_is_html_mail() ) {
		...
		if ( 1 === (int) $usces->options['put_customer_name'] ) {
			// translators: %s: name of user.
			$name     = sprintf( __( 'Dear %s', 'usces' ), usces_localized_name( trim( $user['name1'] ), trim( $user['name2'] ), 'return' ) );
			$message .= $name . '<br>';
		}
		$message .= do_shortcode( wpautop( $mail_data['header']['membercomp'] ) );
		$message .= '</td></tr>';
		// add body.
		$message .= '<tr><td style="padding:20px 30px;">';
		$message .= $msg_body;
