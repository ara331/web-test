...
class USCES_DATALIST_UPGRADE {
	...
	<div class="postbox">
		<div class="postbox-header">
			...
			<div class="handle-actions"><button type="button" class="handlediv" id="datalistup"><span class="screen-reader-text">ÉpÉlÉãÇêÿÇËë÷Ç¶: <?php esc_attr_e( 'Data List Upgrade', 'usces' ); ?></span><span class="toggle-indicator"></span></button></div>
		</div>
		<div class="inside">
		...
		if ( usces_is_tax_display() && 'products' == usces_get_tax_target() ) {
			...
			if ( usces_is_reduced_taxrate() ) {
				if ( isset( $_REQUEST['check']['subtotal_standard'] ) ) {
					$line_col .= $th_h . sprintf( __( 'Applies to %s%%', 'usces' ), $usces_tax->subtotal_standard + $usces_tax->discount_standard ) . $th_f;
				}
				...
				if ( isset( $_REQUEST['check']['subtotal_reduced'] ) ) {
					$line_col .= $th_h . sprintf( __( 'Applies to %s%%', 'usces' ), $usces_tax->subtotal_reduced + $usces_tax->discount_reduced ) . $th_f;
				}
